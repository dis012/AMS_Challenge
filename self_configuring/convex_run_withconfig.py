...
# ####   topk = (3,5,6,10,11,12,13,16)
    ...
    for i in topk:
        ...
        seg_fixed = torch.from_numpy(nib.load(f_gt.replace('xxxx',str(i).zfill(4)).replace('y', '0')).get_fdata()).float().cuda().contiguous()
        segs_fixed.append(seg_fixed)
        seg_fixed = torch.from_numpy(nib.load(f_gt.replace('xxxx',str(i).zfill(4)).replace('y', '1')).get_fdata()).float().cuda().contiguous()
        segs_fixed.append(seg_fixed)
